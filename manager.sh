...
do_init_for_dev() {
    ...
    local config_key="9douyu-loan-user.vars.site_list.$developer_name"
...
to_php() {
    run_cmd "docker exec $docker_run_fg_mode $php_container bash"
}
...
function init_app() {
    local cmd="php artisan migrate --force && php artisan db:seed --force"
...
ALL_COMMANDS="$ALL_COMMANDS init_app"
